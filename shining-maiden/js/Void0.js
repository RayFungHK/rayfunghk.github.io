  ...
  var ElementCollection = (function() {
    ...
    (function() {
      ...
      fn.extend(EventEmitter.prototype, {
        register: function(namespaces, selector, callback, interval) {
          this.callback.push({
          ...
          fn.each(this.callback, function(i, object) {
            if (object.interval != 0) {
              if (object.interval > 0) {
                object.interval--;
                if (object.interval == 0) {
                  delete self.callback.splice(i, 1);
                }
              }
            }
      ...
      function setupEvent(element, events, selector, callback, interval) {
        events = events.split('.');
        ...
        element.eventEmitters[eventName].register(namespaces, selector, callback, interval);
       ...
       * @return {[type]}						[description]
       */
      defaultPrototype.once = function(events, selector, callback) {
        if (fn.isString(events)) {
          events = events.trim();
          if (events) {
            if (selector) {
              ...
              if (fn.isCallable(callback)) {
                fn.each(this, function(i, elem) {
                  fn.each(events, function() {
                    setupEvent(elem, this, selector, callback, 1);
                  });
                });
              }
            }
